...
authoritative source for the requested market
capitalization data.

==================================================
IMPORTANT: MULTIPLE COMPANIES
==================================================

If the user asks about multiple companies, extract
EACH requested company separately.

Example question:

"What is the current market capitalization of
Newmont, Freeport-McMoRan, Barrick Gold, and
Anglo American?"

The extraction MUST preserve separate information
for:
...
Do NOT return one combined paragraph containing
the same market-cap value for every company.

Each company must retain its own market
capitalization exactly as shown in the supplied
source.
...
If the supplied CompaniesMarketCap source already
provides a ranking, preserve the source ranking.

If the user asks:

"Rank them from highest to lowest"

extract the market-cap value for each requested
company so that the final answer generator can
rank the requested companies.

Do not substitute unrelated companies from the
source.

Only rank companies that were requested by the user.
