...
function isRankingQuestion(message: string): boolean {
  ...
  if (containsAny(q, rankingWords)) {
    return true;
  }

  // Ranking by a measurable dimension
  if (
    containsAny(q, rankingDimensions) &&
    containsAny(q, rankingActions)
  ) {
    return true;
  }

  // "companies by revenue", "miners by production", etc.
  if (
    containsAny(q, miningEntities) &&
    containsAny(q, rankingDimensions) &&
    (
      q.includes("by ") ||
      q.includes("with the") ||
      q.includes("according to") ||
      q.includes("based on")
    )
  ) {
    return true;
