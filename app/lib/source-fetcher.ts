...
const MAX_RANKING_SOURCES = 2;
const MAX_RANKING_TEXT = 100000;
...
function isMarketCapRankingQuery(
  message: string
): boolean {
  ...
  return (
    /\bmarket cap\b/.test(text) ||
    /\bmarket capitalization\b/.test(text)
  ) && (
    /\btop\b/.test(text) ||
 ...
 function isRankingQuery(message: string): boolean {
  const text = message.toLowerCase();
  ...
  return rankingTerms.some(pattern =>
    pattern.test(text)
  );
}
